...
from procedural_envs.misc.env import wrap
...
def rollout_env(
  ...
  rng = jax.random.PRNGKey(seed=int(time.time()))
  ...
  states.append(state)
  eval_len = len(rs)
  eval_reward = jnp.concatenate(rs)  #[batch_size=1]
  eval_reward = jnp.sum(eval_reward, axis=0)
  print(f'episode len: {eval_len}, reward {eval_reward}')
  return env, states
    ...
    if batch_size:
      ...
      for i in range(batch_size):
        ...
        html.save_html(
            filename,
            env.sys, [
                jax.tree_util.tree_map(lambda x: x[i], state.qp)
                for state in states
            ],
            make_dir=True)
        if verbose:
