...
def annulus_xy_sampler(rng: jnp.ndarray, r_min: float, r_max: float, init_z: float = 0.0):
  ...
  init_pos = init_pos.at[jnp.index_exp[2]].set(jnp.ones(())*init_z)#jax.ops.index_update(init_pos, jnp.index_exp[2], jnp.ones(())*init_z)
  return init_pos
...
def pitch_yaw_uniform_sampler(rng: jnp.ndarray):
  init_eular = quat2eular(sample_quat_uniform(key=rng))
  # roll -> 0.
  init_eular = init_eular.at[jnp.index_exp[0:1]].set(jnp.zeros(1))#jax.ops.index_update(init_eular, jnp.index_exp[0:1], jnp.zeros(1))
  init_rot = eular2quat(init_eular)
  return init_rot


def yaw_uniform_sampler(rng: jnp.ndarray):
  init_eular = quat2eular(sample_quat_uniform(key=rng))
  # roll, pitch -> 0.
  init_eular = init_eular.at[jnp.index_exp[0:2]].set(jnp.zeros(2))#jax.ops.index_update(init_eular, jnp.index_exp[0:2], jnp.zeros(2))
  # init_eular = jax.ops.index_update(init_eular, jnp.index_exp[2], 0)#jnp.pi/2)
  init_rot = eular2quat(init_eular)
  return init_rot
