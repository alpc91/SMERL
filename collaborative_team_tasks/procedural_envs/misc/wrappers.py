...
class EpisodeWrapper(brax_env.Wrapper):
  ...
  def reset(self, rng: jp.ndarray) -> brax_env.State:
    ...
    state.info['success'] = jp.zeros(rng.shape[:-1])
    return state

  def step(self, state: brax_env.State, action: jp.ndarray) -> brax_env.State:  #大改，如果不是auto_reset,done变成1了还会变成0，需要一个state.info['success']，同时让steps不再增加
    def f(state, _):
    ...
    steps = state.info['steps'] + self.action_repeat*(1.0-state.info['success'])
    one = jp.ones_like(state.done)
    zero = jp.zeros_like(state.done)
    episode_length = jp.array(self.episode_length, dtype=jp.int32)
    state.info['success'] = jnp.logical_or(state.done, state.info['success']).astype(jnp.float32)
    done = jp.where(steps >= episode_length, one, state.done)
    state.info['truncation'] = jp.where(steps >= episode_length,
                                        1 - state.info['success'], zero)
...
class AutoResetWrapper(brax_env.Wrapper):
  ...
  def step(self, state: brax_env.State, action: jp.ndarray) -> brax_env.State:
    if 'steps' in state.info:
      ...
      success = jp.where(state.done, jp.zeros_like(success), success)
      state.info.update(success=success)
    ...
    def where_done(x, y):
      ...
      if type(x) == type([]):
        if done.shape:
          done = jp.reshape(done, [x[0].shape[0]] + [1] * (len(x[0].shape) - 1))  # type: ignore
        return [jp.where(done, x[0], y[0]), jp.where(done, x[1], y[1])]
      else:
        if done.shape:
          done = jp.reshape(done, [x.shape[0]] + [1] * (len(x.shape) - 1))  # type: ignore
        return jp.where(done, x, y)

    reset_state = self.env.reset(None)
    state.info['first_qp']=reset_state.qp
    state.info['first_obs'] = reset_state.obs
  ...
  def reset(self, rng: jp.ndarray) -> brax_env.State:
    ...
    eval_metrics = EvalMetrics(
        episode_metrics=jax.tree_util.tree_map(jp.zeros_like,
                                               reset_state.metrics),
        active_episodes=jp.ones_like(reset_state.done),
        episode_steps=jp.zeros_like(reset_state.done))
