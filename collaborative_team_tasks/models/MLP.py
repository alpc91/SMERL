...
class MLP(linen.Module):
    ...
    def setup(self):
        self.self_net = BaseMLP(input_dim=0,
                                hidden_dim=self.hidden_dim,
                                output_dim=self.vector_dim + self.hidden_dim,
                                activation=self.activation,
                                residual=False,
                                last_act=False,
                                flat=False)
    ...
    def __call__(self, f, s):
        temp_f2s = jnp.concatenate((f, s), axis=-1)  # [self.vector_dim*self.vector_dim+S]
...
class SGNN(linen.Module):
    ...
    edge_index_inner: Optional[jnp.ndarray] = None
    ...
    def setup(self):
        ...
        self.embedding_s = linen.Dense(self.vector_dim,
                        kernel_init=self.kernel_init,
                        use_bias=self.bias)
        ...
        # self.object_message_passing = MLP(vector_dim=self.vector_dim, 
        #                                 hidden_dim=self.vector_dim, 
        #                                 activation=self.activation)
    ...
    def __call__(self, input):
        ...
        x_p, v_p, a_p, h_p = input[..., :3], input[..., 3:6], input[..., 6:9], input[..., 9:]
        ...
        if self.out_dim == 1:
            ...
            f_o = self.embedding_f(f_o.reshape(f_o.shape[0],-1)) # [ N, vd]
            s_o = self.embedding_s(s_o)  # [N=32, H=256]
            ret = jnp.concatenate((f_o.reshape(-1), s_o.reshape(-1)))  # [3, N_obj, 3+1]

            outs = self.predictor(ret)
        else:
            torso_array = jnp.array(self.torso)
            x_o = x_p[self.torso, :]
            ...
            # yaw = yaw_normalized[self.torso,...]
            # # Calculate the rotation matrix
            # cos_yaw = jnp.cos(yaw)
            # sin_yaw = jnp.sin(yaw)
            # zeros = jnp.zeros_like(cos_yaw)
            # mat = construct_3d_basis_from_1_vectors(jnp.stack((cos_yaw, sin_yaw, zeros), axis=-1)) #[4,3,3]

            # f_p = jnp.einsum('bij,bjk->bik', mat[agent_idx[self.obj_id]].transpose(0,2,1), f_p)  # [N,  3, K]

            f_p = self.embedding_f(f_p.reshape(f_p.shape[0],-1)) # [ 3, vd]
            s_p = self.embedding_s(h_p)  # [N=32, H=256]
            f_o = f_p[self.torso, ...]
            s_o = s_p[self.torso, ...]
            outs = []
            for i in range(self.ctl_num):
                f_p_ = jnp.concatenate((f_p[self.torso[i] : self.torso[i+1]], f_o[i][None,...], f_o[task_idx[i]][None,...]), axis=0)

                s_p_ = jnp.concatenate((s_p[self.torso[i] : self.torso[i+1]], s_o[i][None,...], s_o[task_idx[i]][None,...]), axis=0)


                ret = jnp.concatenate((f_p_.reshape(-1), s_p_.reshape(-1)))  # [3, N_obj, 3+1]

                out = self.predictor[i](ret)
