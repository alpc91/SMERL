    ...
    def __call__(self, input):
        ...
        if self.out_dim == 1:
            ...
            for i in range(self.ctl_num):
                ...
                f_p_ = jnp.concatenate((f_p[self.torso[i] : self.torso[i+1]], f_o), axis=0)

                s_p_ = jnp.concatenate((h_p[self.torso[i] : self.torso[i+1]], s_o), axis=0)

                f_p_ = self.embedding_f(f_p_.reshape(f_p_.shape[0],-1)) # [ 3, vd]
                s_p_ = self.embedding_s(s_p_)  # [N=32, H=256]
