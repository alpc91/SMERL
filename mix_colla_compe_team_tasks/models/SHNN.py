...
class SGNN(linen.Module):
    ...
    def __call__(self, input):
        ...
        if self.out_dim == 1:
            ...
            # x_o = x_p[self.torso, :]
            # agent_idx, task_idx, edge_index_bi = bipartite_matching(x_o[:self.ctl_num], x_o[self.ctl_num:self.ctl_num*2], self.ctl_num)

            # center_id = len(self.torso) - 1
            # # Generate an array from 0 to self.ctl_num - 1
            # node_ids = jnp.arange(self.ctl_num)
            # # Create an array of center_id with the same shape as node_ids
            # center_ids = jnp.full_like(node_ids, center_id)
            # # Stack the two arrays to create the center_edges
            # center_edges = jnp.stack([node_ids, center_ids], axis=0).astype(jnp.int32)
            # edge_index_bi = jnp.concatenate([edge_index_bi, center_edges], axis=1)
            # agent_idx = jnp.concatenate([agent_idx, jnp.array([0], dtype = jnp.int32)])
            ...
            f_p = jnp.einsum('bij,bjk->bik', mat[self.obj_id].transpose(0,2,1), f_p)  # [N,  3, K]
            ...
            for i in range(self.ctl_num):
                out = jnp.concatenate((ret[self.torso[i] : self.torso[i+1]]), axis=0)
