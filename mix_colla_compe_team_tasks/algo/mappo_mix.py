...
from models.SHNN import make_sgnn_networks as SHNN
...
def train(environment_fn: Callable[..., envs.Env],
  ...
  def make_models(modelname):
    if modelname == "MLP":
      make_models_fn = MLP
    elif modelname == "SHNN":
      make_models_fn = SHNN

    return make_models_fn
  
  make_models_fn = [make_models(modelname1), make_models(modelname2)]
  ...
  def do_one_step(carry, unused_target_t):
    ...
    alpha = jnp.clip(normalizer_params[0][0]/5000000,0,1)
    nstate = step_fn(state, postprocessed_actions)#, time_steps = normalizer_params[0][0])
    return (nstate, normalizer_params, policy_params,
            key), StepData(
                obs=state.core.obs,
                rewards=alpha*state.core.reward + (1-alpha)*state.core.score,
  ...
  def generate_unroll(carry, unused_target_t):
    ...
    data = data.replace(
        ...
        rewards=jnp.concatenate(
            [data.rewards,
             jnp.expand_dims(alpha*state.core.reward + (1-alpha)*state.core.score, axis=0)]),
...
def make_inference_fn(
    ...
    .NormalTanhDistribution,
    make_models_fn: List[Optional[Callable[
        [int, int], Tuple[networks.FeedForwardNetwork]]]] = List[networks.make_models]):
  ...
  for i, (k, action_shape) in enumerate(action_shapes.items()):
    ...
    policy_model, _ = make_models_fn[i](obs_size = obs_size[i], local_state_size=local_state_size[i],  num_node = num_node[i], ctl_num = ctl_num[i], torso = torso[i], edge_index = edge_index[i], edge_type = edge_type[i], obj_id=obj_id[i], policy_params_size = parametric_action_distribution.param_size, edge_index_inner = edge_index_inner[i])
