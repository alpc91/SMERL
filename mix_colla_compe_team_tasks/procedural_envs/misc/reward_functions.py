...
def sumo_distance_reward(action: jnp.ndarray,
                          obs_dict: Dict[str, jnp.ndarray],
                          target: Union[Observer, jnp.ndarray],
                          obs: List[Observer],
                          max_dist: float = 1e8,
  ...
  del action
  norm_kwargs = norm_kwargs or {}
  target = index_obs_dict(obs_dict, target)
  dists = jnp.array([
      jnp.linalg.norm(
  ...
  # return -dist, done
  return jnp.exp(dist-max_dist), done

def nearest_distance_reward(action: jnp.ndarray,
                            obs_dict: Dict[str, jnp.ndarray],
                            target: Union[Observer, jnp.ndarray],
                            obs: List[Observer],
                            max_dist: float = 1e8,
...
def nearest_distance_reward_2(action: jnp.ndarray,
                            obs_dict: Dict[str, jnp.ndarray],
                            target0: Union[Observer, jnp.ndarray],
                            target1: Union[Observer, jnp.ndarray],
                            obs: List[Observer],
                            max_dist: float = 1e8,
                            min_dist: float = 0,
                            norm_kwargs: Dict[str, Any] = None):
  ...
  del action
  norm_kwargs = norm_kwargs or {}
  target0 = index_obs_dict(obs_dict, target0)
  target1 = index_obs_dict(obs_dict, target1)
  dists0 = jnp.array([
      jnp.linalg.norm(
...
def nearest_distance_reward_3(action: jnp.ndarray,
                            ...
                            target0: Union[Observer, jnp.ndarray],
                            target1: Union[Observer, jnp.ndarray],
                            target2: Union[Observer, jnp.ndarray],
                            obs: List[Observer],
                            max_dist: float = 1e8,
                            min_dist: float = 0,
                            norm_kwargs: Dict[str, Any] = None):
  ...
  target0 = index_obs_dict(obs_dict, target0)
  target1 = index_obs_dict(obs_dict, target1)
  target2 = index_obs_dict(obs_dict, target2)
  dists0 = jnp.array([
      jnp.linalg.norm(
          index_obs_dict(obs_dict, o) - target0,
          axis=-1,
