...
class GraphObserver(Observer):
  ...
  def initialize(self, group, sys, agent_names, task_edge=[[],[],[]]):
    ...
    self.node_names2 = [body for body in sim_utils.get_names(sys.config, 'body') if 'agent' in body and body not in child_dict.keys() and group not in body]  # removing Ground and body 
    self.node_center = [body for body in sim_utils.get_names(sys.config, 'body') if 'center' in body]  # removing Ground
    ...
    for agent in agent_names:
      if group not in agent:
        self.team_id = jnp.concatenate([self.team_id, jnp.ones(1, dtype=jnp.int32)])
        self.torso.append(idx)
        idx+=1

    for name in self.node_center:
      self.team_id = jnp.concatenate([self.team_id, 2*jnp.ones(1, dtype=jnp.int32)])
      self.torso.append(idx)
      idx += 1        
    ...
    for node_idx, node in enumerate(self.node_names2):
      if node in child_dict.keys():
        ...
        parent = child_dict[node][2]
        if 'torso' in parent:
          j_p_idx = self.torso[extract_agent_id(parent)+self.ctl_num]
        else:
          j_p_name = child_dict[parent][1]
          j_p_dof = self.joint_idx_dict[j_p_name]['dof']
          j_p_idx = self.joint_idx_dict[j_p_name]['idx']
  ...
  def _get_obs(self,
    ...
    def get_obs_fn(obs_names, type_obj):
      ...
      for node_idx, node in enumerate(obs_names):
        ...
        local_h_append(jnp.eye(3)[type_obj])
        # add one_hot_limb_id
    ...
    get_obs_fn(self.node_names2,1)
    get_obs_fn(self.node_center,2)
