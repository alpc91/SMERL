...
from procedural_envs.misc import component_editor as ce
...
def process_agent_rewards(metadata: Any,
  ...
  all_reward_names = ()
  # if metadata.satisfy_all_cond:
  #   done = jnp.ones((num_agents,))
  for i, (_, v) in enumerate(sorted(metadata.agent_groups.items())):
    ...
    for reward_name in reward_names:
      ...
      if 'win' in reward_name or 'lose' in reward_name:
      ...
      # if metadata.satisfy_all_cond:
      #   done = done.at[i].set(jnp.logical_and(done[i], d))
      # else:
...
def set_names_info(
  ...
  # assert all(isinstance(name, tuple)
  #            for name in names), f'{names} must be a Sequence of Tuples'
  if all(isinstance(name, tuple)
             for name in names):
    ...
    for agent in agents:
      ...
      names += tuple(k for k in all_names if ce.match_group_name(k, *agent_args))
