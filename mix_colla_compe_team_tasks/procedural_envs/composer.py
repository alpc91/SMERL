...
from procedural_envs.misc import observers, agent_utils
...
class CustomComposer(object):
  ...
  def reset_fn(self, sys, qp: brax.v1.physics.base.QP, rng: jnp.ndarray):
    # 1. 划分角度区间: 总共有12个区间，每个30度
    angle_intervals = jnp.linspace(0, 360, 9)[:-1]  # [0, 30, 60, ..., 330]
    # print("angle_intervals",angle_intervals)

    # 生成奇数和偶数间隔的选择
    even_intervals = angle_intervals[0::2]  # 偶数间隔
    odd_intervals = angle_intervals[1::2]  # 奇数间隔

    # 2. 随机选择是从奇数还是偶数区间开始采样
    rng, sub_rng = jax.random.split(rng)
    # start_from_even = jax.random.choice(sub_rng, jnp.array([0, 1]))
    start_from_even = jax.random.choice(sub_rng, jnp.array([0, 1]), shape=()) > 0
    # 根据选择使用奇数或偶数间隔
    selected_intervals = jax.lax.cond(start_from_even,
                        lambda _: even_intervals,
                        lambda _: odd_intervals,
                        operand=None)
    
    #  even_intervals# if start_from_even == 0 else odd_intervals
    # print("selected_intervals",selected_intervals)

    # 3. 随机确定区间排列
    rng, sub_rng = jax.random.split(rng)
    shuffled_intervals = jax.random.permutation(sub_rng, selected_intervals)
    # print("shuffled_intervals",shuffled_intervals)
    i = 0
    ...
    for _, v in sorted(self.metadata.components.items()):
      ...
      if v['random_init']:
        ...
        if v['random_init'] == 'posquat':
          rng, sub_rng = jax.random.split(rng)
          random_init_pos_fn = functools.partial(v['random_init_pos_fn'], rng=sub_rng)
          random_init_quat_fn = functools.partial(v['random_init_quat_fn'], rng=sub_rng)

          interval_index = i % len(shuffled_intervals)
          rng, sub_rng = jax.random.split(rng)
          angle = jax.random.uniform(sub_rng, minval=shuffled_intervals[interval_index], maxval=shuffled_intervals[interval_index] + 30)
          rng, sub_rng = jax.random.split(rng)
          radius = jax.random.uniform(sub_rng, minval=1.0, maxval=2.0)

          # 将极坐标转换为笛卡尔坐标
          x = jnp.array([radius * jnp.cos(jnp.deg2rad(angle))])
          y = jnp.array([radius * jnp.sin(jnp.deg2rad(angle))])
          # print(radius,x,y)

          init_pos = jnp.concatenate([x, y])
          # xy -> xyz
          init_pos = jnp.zeros(3).at[jnp.index_exp[0:2]].set(init_pos)#jax.ops.index_update(jnp.zeros(3), jnp.index_exp[0:2], init_pos)
          ...
          qp = sim_utils.transform_qp(qp, mask[..., None], random_init_quat_fn(),
                                    jnp.zeros(3), init_pos)#, v['pos']  #modify 2024.1.1
...
class CustomComponentEnv(Env):
  ...
  def __init__(self,
    ...
    self.observer2.initialize('team1', self.sys, self.agent_names)#, env_desc['task_edge'])
    ...
    self.team_id = self.observer.team_id
    self.team_id2 = self.observer2.team_id
    self.edge_index = self.observer.edge_index
    self.edge_index2 = self.observer2.edge_index
    self.edge_index_inner = self.observer.edge_index_inner
    self.edge_index_inner2 = self.observer2.edge_index_inner
  ...
  def step(self,
    ...
    return state.replace(
        qp=qp, obs=obs, reward=reward, score = score, done=done.astype(jnp.float32))  #modify 2024.1.1
